...
export const DimensionModal: React.FC<DimensionModalProps> = ({
  ...
  useEffect(() => {
    const fetchDimensionDetails = async () => {
      try {
        ...
        // For Auto mode, automatically process and save
        if (data.processing_mode === 'Auto' && submission && !isSubmitted) {
          setCriteriaData(submission.criteria_data);
          const result = await generateAndSaveContent(data.ai_function as AIFunction, submission.criteria_data);
          if (result) {
            onSubmit();
          }
        }
  ...
  const generateAndSaveContent = async (aiFunction: AIFunction, criteriaData: Record<string, any>) => {
    try {
      setIsLoading(true);
      ...
      // Configure request based on AI function
      switch (aiFunction) {
        case 'generate-text':
      ...
      if (endpoint) {
        ...
        // Format AI response based on function type
        switch (aiFunction) {
          case 'process-text-to-pdf':
            bboxData.message = aiResponse.message;
            break;
          case 'generate-descriptions':
            bboxData.descriptions = aiResponse;
            break;
          case 'generate-axes':
            bboxData.axes = aiResponse.axes;
            atelierData.text = criteriaValues[0] || "";
            break;
          case 'sum-letters':
            bboxData.code = aiResponse.sums?.filter(num => !isNaN(num)).join('');
            break;
          case 'transform-dict':
            bboxData.code = formatTransformDictResponse(aiResponse);
            break;
          default:
            bboxData.text = aiResponse.text;
        }

        // Add criteria data if not generate-axes
        if (aiFunction !== 'generate-axes') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // For Auto mode dimensions, process and save immediately
      if (dimension?.processing_mode === 'Auto') {
        await generateAndSaveContent(selectedFunction, criteriaData);
        onSubmit();
        return;
      }

      // For Manual mode, just generate content
  ...
  const handleVerify = async () => {
    try {
      ...
      // For Auto mode, process and save automatically
      if (dimension?.processing_mode === 'Auto') {
        await generateAndSaveContent(selectedFunction, criteriaData);
        onSubmit();
        return;
      ...
      // Manual mode continues with existing flow
      const criteriaValues = Object.values(criteriaData);
      let endpoint = '';
      let requestBody = {};
      const bboxData: Record<string, any> = {};
      ...
      if (endpoint && !generatedContent) {
        ...
        // Format response based on AI function type
        switch (selectedFunction) {
          case 'process-text-to-pdf':
            bboxData.message = generatedContent.message;
            break;
          case 'generate-descriptions':
            bboxData.descriptions = generatedContent;
            break;
          case 'generate-axes':
            bboxData.axes = generatedContent.axes;
            atelierData.text = criteriaValues[0] || "";
            break;
          case 'sum-letters':
            bboxData.code = generatedContent.sums?.filter(num => !isNaN(num)).join('');
            break;
          case 'transform-dict':
            bboxData.code = formatTransformDictResponse(generatedContent);
            break;
          default:
            bboxData.text = generatedContent.text;
        }
